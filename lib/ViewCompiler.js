...
ViewCompiler.prototype = {
	...
	compile: function(viewFile) {
		...
		var stylesheets = [];
		var currentTagName;
		dependencies.push('var viewManager = require("ViewManager");');
//		if (viewFile.indexOf('11') > -1) { console.log(JSON.stringify(this._tokenize(this._getFileContents(viewFile)))); }
		this._tokenize(this._getFileContents(viewFile)).forEach(function(token) {
			if (token.type == 'link' && token.rel == 'require') {
				...
				js += 'viewManager.addView(view' + viewIds[viewIds.length-1] + ', options.id, options.class, "' + token.tagName + '", ' + (uid > 1 ? 'view' + viewIds[viewIds.length-2] : 'null') + ');';
				uid++;
			}
...
if (viewFile.indexOf('9') > -1) { console.log('***js***\n\n' + js); }
